...
const DropDownButton: React.FC<DropDownButtonProps<SortEvents | SortDateRange>> = ({
  ...
  useEffect(() => {
    // I don't like that this gets passed as string.
  ...
  return (
    ...
    >
      <div>
        <Menu.Button className="inline-flex w-full bg-brand-blue justify-between gap-x-1.5 whitespace-nowrap rounded-xl px-3 py-2 text-xs font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
          {`${name} ${selected}`}
          <ChevronDownIcon
